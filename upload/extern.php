...
// The length at which topic messages will be truncated (for news output)
...
if ($_GET['action'] == 'active' || $_GET['action'] == 'new')
{
	...
	if (isset($_GET['type']) && strtoupper($_GET['type']) == 'RSS')
	{
		...
		while ($cur_topic = $db->fetch_assoc($result))
		{
			...
			$cur_message = !is_null($cur_message) ? $cur_message : '';
	...
	// Should we use template when output news?
	elseif (isset($_GET['type']) && $_GET['type'] == 'gen_news')
	{
...
//Change Ragnaar line 266		$result = $db->query('SELECT t.id, t.poster, t.subject, t.num_replies FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id=3) WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.moved_to IS NULL'.$forum_sql.' ORDER BY '.$order_by.' DESC LIMIT '.$show) or error('Unable to fetch topic list', __FILE__, __LINE__, $db->error());
		...
		while ($cur_topic = $db->fetch_assoc($result))
		{
			...
			if ($cur_topic['question'] != '')
				$replace = ['<strong>'.$lang_polls['Poll'].': '.pun_htmlspecialchars($cur_topic['subject']).'</strong>', $lang_common['Author'].': '.$cur_topic['poster'], date('d-m-Y', (int)$cur_topic['posted']), $message_truncated, '<a href="'.$pun_config['o_base_url'].'/viewtopic.php?id='.$cur_topic['id'].'">'.$lang_common['Comments'].':'.$cur_topic['num_replies'].'</a>', $pun_config['o_base_url'].'/viewtopic.php?id='.$cur_topic['id']];
			else
				$replace = [pun_htmlspecialchars($cur_topic['subject']), $lang_common['Author'].': '.$cur_topic['poster'], date('d-m-Y', (int)$cur_topic['posted']), $message_truncated, '<a href="'.$pun_config['o_base_url'].'/viewtopic.php?id='.$cur_topic['id'].'">'.$lang_common['Comments'].':'.$cur_topic['num_replies'].'</a>', $pun_config['o_base_url'].'/viewtopic.php?id='.$cur_topic['id']];
...
//Change Ragnaar line 288 			$search = array('<news_subject>', '<news_author>', '<news_posted>', '<news_message>', '<news_comments>');


			echo str_replace($search, $replace, $news_tpl);
		}
	}
...
else if ($_GET['action'] == 'online' || $_GET['action'] == 'online_full')
{
	...
	while ($pun_user_online = $db->fetch_assoc($result))
	{
		if ($pun_user_online['user_id'] > 1)
		{
			...
			if ($pun_user_online['show_online'] == '0') ++$num_hidden;
		}
		else
			++$num_guests;
	}
