...
if (isset($_POST['form_sent']) && $action == 'in')
{
	...
	if (!empty($db_password_hash))
	{
		...
		else if (!$sha1_in_db && $db_password_hash == md5($form_password))
		{
			...
			$db->query('DELETE FROM '.$db->prefix.'iptrylog WHERE ip=\''.get_remote_address().'\'') or error('Unable to update user status', __FILE__, __LINE__, $db->error());
			// End of Part 2.
...
else if ($action == 'forget' || $action == 'forget_2')
{
	...
	if (isset($_POST['form_sent']))
	{
		...
		$result = $db->query('SELECT id, username FROM '.$db->prefix.'users WHERE email=\''.$db->escape($email).'\' AND group_id > '.PUN_MOD) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
