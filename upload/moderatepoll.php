...
if (isset($_GET['tid']))
{
	...
	if (isset($_POST['delete_posts']) || isset($_POST['delete_posts_comply']))
	{
		...
		{
			confirm_referrer('moderatepoll.php');
			...
			redirect('viewpoll.php?id='.$tid, $lang_misc['Delete posts redirect']);
		}
...
<div class="blockform">
	...
	<div class="box">
		<form method="post" action="moderatepoll.php?fid=<?php echo $fid ?>&amp;tid=<?php echo $tid ?>">
			<div class="inform">
				<fieldset>
...
<?php
	...
	$paging_links = $lang_common['Pages'].': '.paginate($num_pages, $p, 'moderatepoll.php?fid='.$fid.'&amp;tid='.$tid);
...
if (isset($_REQUEST['move_topics']) || isset($_POST['move_topics_to']))
{
	...
	$page_title = 'Moderate | '.pun_htmlspecialchars($pun_config['o_board_title']);
	require PUN_ROOT.'header.php';
...
<div class="blockform">
	...
	<div class="box">
		<form method="post" action="moderatepoll.php?fid=<?php echo $fid ?>">
			<div class="inform">
...
else if (isset($_REQUEST['open']) || isset($_REQUEST['close']))
{
	...
	if (isset($_POST['open']) || isset($_POST['close']))
	{
		...
		redirect('moderatepoll.php?fid='.$fid, $redirect_msg);
	}
	// Or just one in $_GET
	else
	{
		confirm_referrer('viewpoll.php');
		...
		redirect('viewpoll.php?id='.$topic_id, $redirect_msg);
	}
}
...
else if (isset($_GET['stick']))
{
	...
	redirect('viewpoll.php?id='.$stick, $lang_misc['Stick topic redirect']);
}
...
else if (isset($_GET['unstick']))
{
	...
	redirect('viewpoll.php?id='.$unstick, $lang_misc['Unstick topic redirect']);
}
...
// Generate paging links
$paging_links = $lang_common['Pages'].': '.paginate($num_pages, $p, 'moderatepoll.php?fid='.$fid)
...
if ($db->num_rows($result))
{
	...
	while ($cur_topic = $db->fetch_assoc($result))
	{
		...
		{
			$last_post = '<a href="viewpoll.php?pid='.$cur_topic['last_post_id'].'#p'.$cur_topic['last_post_id'].'">'.format_time($cur_topic['last_post']).'</a> '.$lang_common['by'].' '.pun_htmlspecialchars($cur_topic['last_poster']);
			$ghost_topic = false;
		}
		else
		{
		...
		if ($cur_topic['moved_to'] != 0)
			$subject = $lang_forum['Moved'].': <a href="viewpoll.php?id='.$cur_topic['moved_to'].'">'.pun_htmlspecialchars($cur_topic['subject']).'</a> <span class="byuser">'.$lang_common['by'].' '.pun_htmlspecialchars($cur_topic['poster']).'</span>';
		else if ($cur_topic['closed'] == '0')
			$subject = '<a href="viewpoll.php?id='.$cur_topic['id'].'">'.pun_htmlspecialchars($cur_topic['subject']).'</a> <span>'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($cur_topic['poster']).'</span>';
		else
		{
			$subject = '<a href="viewpoll.php?id='.$cur_topic['id'].'">'.pun_htmlspecialchars($cur_topic['subject']).'</a> <span class="byuser">'.$lang_common['by'].' '.pun_htmlspecialchars($cur_topic['poster']).'</span>';
