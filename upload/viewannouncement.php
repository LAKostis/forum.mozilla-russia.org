...
else if ($action == 'new' && !$pun_user['is_guest']) 
{
    ...
    if ($first_new_post_id)
        header('Location: viewannouncement.php?pid=' . $first_new_post_id . '#p' . $first_new_post_id);
    else // If there is no new post, we go to the last post
        header('Location: viewannouncement.php?id=' . $id . '&action=last');
...
else if ($action == 'last') 
{
    ...
    if ($last_post_id) 
{
        header('Location: viewannouncement.php?pid=' . $last_post_id . '#p' . $last_post_id);
        exit;
    } 
} 
...
if (!$pun_user['is_guest'])
    $result = $db->query('SELECT t.subject, t.closed, t.num_replies FROM ' . $db->prefix . 'topics AS t WHERE t.id=' . $id . ' AND t.announcement=\'1\' AND t.moved_to IS NULL') or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
else
    $result = $db->query('SELECT t.subject, t.closed, t.num_replies FROM ' . $db->prefix . 'topics AS t WHERE t.id=' . $id . ' AND t.announcement=\'1\' AND t.moved_to IS NULL') or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
...
$is_admmod = ($pun_user['g_id'] == PUN_ADMIN) ? true : false;
// Can we or can we not post replies?
if ($cur_topic['closed'] == '0') {
    if (($pun_user['g_post_replies'] == '1') || $is_admmod)
        $post_link = '<a href="announcementreply.php?tid=' . $id . '">' . $lang_topic['Post reply'] . '</a>';
    else
        $post_link = '&nbsp;';
} 
else 
...
$paging_links = $lang_common['Pages'] . ': ' . paginate($num_pages, $p, 'viewannouncement.php?id=' . $id);
...
<div class="linkst">
	<div class="inbox">
		...
		<ul><li><a href="index.php"><?php echo $lang_common['Index'] ?></a></li><li>&nbsp;&raquo;&nbsp;<?php echo $lang_common['Announcements'] ?></li><li>&nbsp;&raquo;&nbsp;<?php echo pun_htmlspecialchars($cur_topic['subject']) ?></li></ul>
    ...
    if (!$is_admmod) {
        ...
        if ($cur_topic['closed'] == '0') {
            if ($cur_post['poster_id'] == $pun_user['id']) {
					...
					$post_actions[] = '<li class="postdelete"><a href="announcementdelete.php?id='.$cur_post['id'].'">'.$lang_topic['Delete'].'</a>';
				if ($pun_user['g_edit_posts'] == '1')
					$post_actions[] = '<li class="postedit"><a href="announcementedit.php?id='.$cur_post['id'].'">'.$lang_topic['Edit'].'</a>';
			}

			if ($pun_user['g_post_replies'] == '1')
				$post_actions[] = '<li class="postquote"><a href="announcementreply.php?tid='.$id.'&amp;qid='.$cur_post['id'].'">'.$lang_topic['Quote'].'</a>';
		}
	}
	else
		$post_actions[] = '<li class="postdelete"><a href="announcementdelete.php?id='.$cur_post['id'].'">'.$lang_topic['Delete'].'</a>'.$lang_topic['Link separator'].'</li><li class="postedit"><a href="announcementedit.php?id='.$cur_post['id'].'">'.$lang_topic['Edit'].'</a>'.$lang_topic['Link separator'].'</li><li class="postquote"><a href="announcementreply.php?tid='.$id.'&amp;qid='.$cur_post['id'].'">'.$lang_topic['Quote'].'</a>';
...
<div class="blockform">
	...
	<div class="box">
		<form method="post" action="announcementreply.php?tid=<?php echo $id ?>" onsubmit="this.submit.disabled=true;if(process_form(this)){return true;}else{this.submit.disabled=false;return false;}">
			<div class="inform">
				<fieldset>
