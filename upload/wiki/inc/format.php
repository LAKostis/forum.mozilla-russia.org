...
function format_link_interwiki($link){
  ...
  if(@file_exists('wiki/interwiki/'.$ico.'.png')){
    $link['style']='background: transparent url('.getBaseURL().'wiki/interwiki/'.$ico.'.png) 0px 1px no-repeat;';
  }elseif(@file_exists('wiki/interwiki/'.$ico.'.gif')){
    $link['style']='background: transparent url('.getBaseURL().'wiki/interwiki/'.$ico.'.gif) 0px 1px no-repeat;';
  }
...
function format_link_media($link){
  ...
  if($isimg){
    ...
    if($a) $link['name'] .= $a;
           $link['name'] .= ' class="'.$class.'" style="border:0" alt="'.$t.'" />';
  }else{
    if($t){
      $link['name'] = $t;
    }else{
...
function format_rss($url){
  ...
  define('MAGPIE_DIR', 'wiki/inc/magpie/');
