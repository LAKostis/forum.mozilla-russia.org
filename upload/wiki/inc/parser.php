 ...
 */
  ...
  include_once("wiki/inc/common.php");
  include_once("wiki/inc/html.php");
  include_once("wiki/inc/format.php");
  require_once("lang/English/lang.php");
  @include_once("lang/".$conf['lang']."/lang.php");
...
function acronyms(&$table,&$text){
  $acronyms = file('wiki/conf/acronyms.conf');
...
function customs($text){
  $reps = file ('wiki/conf/custom.conf');
  foreach($reps as $rep){
  ...
  switch ($type){
    ...
    case 'code':
      if(empty($option)){
        ...
        require_once("wiki/inc/geshi.php");
        $geshi = new GeSHi($text, strtolower($option), "wiki/inc/geshi");
